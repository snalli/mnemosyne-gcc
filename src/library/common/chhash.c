...
m_result_t m_chhash_create(m_chhash_t **hp, unsigned int table_size, bool mtsafe) {
    ...
    *hp = (m_chhash_t *)MALLOC(sizeof(m_chhash_t));
    ...
    (*hp)->tbl = (m_chhash_bucket_list_t *)CALLOC(table_size, sizeof(m_chhash_bucket_list_t));
    if ((*hp)->tbl == NULL) {
        FREE(*hp);
        return M_R_NOMEMORY;
    }
...
static void bucket_list_add(m_chhash_bucket_t **head, m_chhash_bucket_t *bucket) {
    if (*head) {
        (*head)->prev = bucket;
    }
    bucket->next = *head;
...
static void bucket_list_remove(m_chhash_bucket_t **head, m_chhash_bucket_t *bucket) {
    if (*head == bucket) {
        *head = bucket->next;
        bucket->prev = NULL;
    } else {
...
m_result_t m_chhash_add(m_chhash_t *h, m_chhash_key_t key, m_chhash_value_t value) {
    ...
    for (bucket = bucket_list->head; bucket != NULL; bucket = bucket->next) {
        if (bucket->key == key) {
            result = M_R_EXISTS;
            goto done;
        }
    ...
    for (i = 0; i < h->tbl_size; i++) {
        ...
        if ((bucket = bucket_list->head)) {
            ...
            for (; bucket != NULL; bucket = bucket->next) {
                fprintf(M_DEBUG_OUT, " --> (%lu, %p)", (long unsigned int)bucket->key,
                        bucket->value);
            }
            fprintf(M_DEBUG_OUT, "\n");
        }
