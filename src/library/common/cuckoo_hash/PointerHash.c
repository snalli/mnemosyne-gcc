...
// metadoc PointerHash notes Suggestion to use cuckoo hash and original implementation by Marc
// Fauconneau
...
IOINLINE void PointerHash_setSize_(PointerHash *self, size_t size) {
    ...
    if (size > self->size) {
        PM_MEMSET(self->records + self->size * sizeof(PointerHashRecord), 0x0,
...
IOINLINE void PointerHash_insertRecords(PointerHash *self, unsigned char *oldRecords,
                                        size_t oldSize) {
    size_t i;
...
IOINLINE void PointerHash_compact(PointerHash *self) {}
