...
// metadoc PointerSetHash notes Suggestion to use cuckoo hash and original implementation by Marc
// Fauconneau
...
void PointerSetHash_setSize_(PointerSetHash *self, size_t size) {
    ...
    if (size > self->size) {
        PM_MEMSET(self->records + self->size * sizeof(PointerSetHashRecord), 0x0,
