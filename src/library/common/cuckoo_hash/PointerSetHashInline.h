...
static inline BASEKIT_API void PointerSetHash_copy_(PointerSetHash *self,
                                                    const PointerSetHash *other);
...
static inline BASEKIT_API void PointerSetHash_insert_(PointerSetHash *self,
...
#define PointerSetHashRecords_recordAt_(records, pos)                                              \
    (PointerSetHashRecord *)(records + (pos * sizeof(PointerSetHashRecord)))
...
IOINLINE void PointerSetHashRecord_swapWith_(PointerSetHashRecord *self,
                                             PointerSetHashRecord *other) {
...
IOINLINE void PointerSetHash_setSize_(PointerSetHash *self, size_t size) {
    ...
    if (size > self->size) {
        memset(self->records + self->size * sizeof(PointerSetHashRecord), 0x0,
               (size - self->size) * sizeof(PointerSetHashRecord));
    }
...
IOINLINE void PointerSetHash_insertRecords(PointerSetHash *self, unsigned char *oldRecords,
                                           size_t oldSize) {
    size_t i;
