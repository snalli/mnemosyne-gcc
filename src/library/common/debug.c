...
#define PSEGMENT_RESERVED_REGION_END                                                               \
...
void __pm_trace_print(char *format, ...) {
    ...
    if (!strcmp(marker, PM_FENCE_MARKER) || !strcmp(marker, PM_TX_END)) {
        /*
    ...
    } else if (!strcmp(marker, PM_WRT_MARKER) || !strcmp(marker, PM_DWRT_MARKER) ||
               !strcmp(marker, PM_DI_MARKER) || !strcmp(marker, PM_NTI)) {
        addr = va_arg(__va_list, unsigned long long);

        if ((PSEGMENT_RESERVED_REGION_START < addr && addr < PSEGMENT_RESERVED_REGION_END))
            reg_write = 1;
    } else { /* empty */
    }
    va_end(__va_list);
...
void m_debug_print_L(int debug_flag, const char *strformat, ...) {
    char msg[512];
    int len;
    va_list ap;
    ...
    len = sprintf(msg, "[M_DEBUG: T-%02u (%u) TS=%04u%06u TX=%d PC=%p] ", tid, tid_pthread,
                  (unsigned int)curtime.tv_sec, (unsigned int)curtime.tv_usec, xact_state,
