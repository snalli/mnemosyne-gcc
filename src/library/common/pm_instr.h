...
#define time_since_start                                                                           \
    ({                                                                                             \
        ...
        (((1000000 * mtm_time.tv_sec + mtm_time.tv_usec) - (glb_start_time)) - buf_drain_period);  \
    })

#define TENTRY_ID                                                                                  \
    (mtm_tid == -1 ? ({                                                                            \
        mtm_tid = syscall(SYS_gettid);                                                             \
        mtm_tid;                                                                                   \
    })                                                                                             \
                   : mtm_tid),                                                                     \
        (time_since_start)
    ...
    {                                                                                              \
        if (mtm_enable_trace) {                                                                    \
            ...
            if (tsz < MAX_TBUF_SZ - tbuf_sz) {                                                     \
                memcpy(tbuf + tbuf_sz, tstr, tsz);                                                 \
                tbuf_sz += tsz;                                                                    \
            } else {                                                                               \
                tbuf_ptr = 0;                                                                      \
                if (mtm_debug_buffer) {                                                            \
                    fprintf(m_err, "start_buf_drain - %llu us\n",                                  \
                ...
                if (mtm_debug_buffer) {                                                            \
                    fprintf(m_err, "end_buf_drain - %llu us\n",                                    \
                            (end_buf_drain = time_since_start));                                   \
                }                                                                                  \
                memcpy(tbuf, tstr, tsz);                                                           \
                tbuf_sz += tsz;                                                                    \
                buf_drain_period += (end_buf_drain - start_buf_drain) + 1;                         \
            }                                                                                      \
...
#define PM_STORE(pm_dst, bytes)                                                                    \
    ({                                                                                             \
        PM_TRACE("%d:%llu:%s:%p:%lu:%s:%d\n", TENTRY_ID, PM_WRT_MARKER, (pm_dst), bytes, LOC1,     \
                 LOC2);                                                                            \
    })

#define PM_WRITE(pm_dst)                                                                           \
    ({                                                                                             \
        PM_TRACE("%d:%llu:%s:%p:%lu:%s:%d\n", TENTRY_ID, PM_WRT_MARKER, &(pm_dst),                 \
                 sizeof((pm_dst)), LOC1, LOC2);                                                    \
                                                                                                   \
        pm_dst;                                                                                    \
    })
...
#define PM_EQU_DW(pm_dst, y)                                                                       \
    ({                                                                                             \
        PM_TRACE("%d:%llu:%s:%p:%lu:%s:%d\n", TENTRY_ID, PM_DWRT_MARKER, &(pm_dst),                \
                 sizeof((pm_dst)), LOC1, LOC2);                                                    \
        pm_dst = y;                                                                                \
    })
...
#define PM_MEMSET(pm_dst, val, sz)                                                                 \
    ({                                                                                             \
        PM_TRACE("%d:%llu:%s:%p:%lu:%s:%d\n", TENTRY_ID, PM_WRT_MARKER, (pm_dst),                  \
                 (unsigned long)sz, LOC1, LOC2);                                                   \
        do {                                                                                       \
            void *_p = (void *)(pm_dst);                                                           \
            memset(_p, val, (size_t)(sz));                                                         \
        } while (0);                                                                               \
    })
...
#define PM_STRCPY(pm_dst, src)                                                                     \
    ({                                                                                             \
        PM_TRACE("%d:%llu:%s:%p:%u:%s:%d\n", TENTRY_ID, PM_WRT_MARKER, (pm_dst),                   \
                 (int)strlen((src)), LOC1, LOC2);                                                  \
        strcpy(pm_dst, src);                                                                       \
    })

#define PM_MOVNTI(pm_dst, count, copied)                                                           \
    ({                                                                                             \
        PM_TRACE("%d:%llu:%s:%p:%lu:%lu:%s:%d\n", TENTRY_ID, PM_NTI, (pm_dst),                     \
                 (unsigned long)copied, (unsigned long)count, LOC1, LOC2);                         \
        0;                                                                                         \
    })
...
#define PM_READ(pm_src)                                                                            \
    ({                                                                                             \
        PM_TRACE("%d:%llu:%s:%p:%lu:%s:%d\n", TENTRY_ID, PM_RD_MARKER, &(pm_src),                  \
                 sizeof((pm_src)), LOC1, LOC2);                                                    \
        (pm_src);                                                                                  \
    })
...
#define PM_RD_WR_P(pm_src)                                                                         \
    ({                                                                                             \
        PM_TRACE("%d:%llu:%s:%p:%lu:%s:%d\n", TENTRY_ID, PM_RD_MARKER, &(pm_src),                  \
                 sizeof((pm_src)), LOC1, LOC2);                                                    \
        PM_TRACE("%d:%llu:%s:%p:%lu:%s:%d\n", TENTRY_ID, PM_WRT_MARKER, &(pm_src),                 \
                 sizeof((pm_src)), LOC1, LOC2);                                                    \
        &(pm_src);                                                                                 \
    })
...
#define start_txn() ({ PM_TRACE("%d:%llu:%s:%s:%d\n", TENTRY_ID, PM_TX_START, LOC1, LOC2); })

#define end_txn() ({ PM_TRACE("%d:%llu:%s:%s:%d\n", TENTRY_ID, PM_TX_END, LOC1, LOC2); })
...
#define PM_FLUSH(pm_dst, count, done)                                                              \
    ({                                                                                             \
        PM_TRACE("%d:%llu:%s:%p:%u:%u:%s:%d\n", TENTRY_ID, PM_FLUSH_MARKER, (pm_dst), done, count, \
                 LOC1, LOC2);                                                                      \
    })

#define PM_FLUSHOPT(pm_dst, count, done)                                                           \
    ({                                                                                             \
        PM_TRACE("%d:%llu:%s:%p:%u:%u:%s:%d\n", TENTRY_ID, PM_FLUSHOPT_MARKER, (pm_dst), done,     \
                 count, LOC1, LOC2);                                                               \
    })

#define PM_COMMIT() ({ PM_TRACE("%d:%llu:%s:%s:%d\n", TENTRY_ID, PM_COMMIT_MARKER, LOC1, LOC2); })
#define PM_BARRIER() ({ PM_TRACE("%d:%llu:%s:%s:%d\n", TENTRY_ID, PM_BARRIER_MARKER, LOC1, LOC2); })
#define PM_FENCE() ({ PM_TRACE("%d:%llu:%s:%s:%d\n", TENTRY_ID, PM_FENCE_MARKER, LOC1, LOC2); })
