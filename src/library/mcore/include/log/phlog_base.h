 ...
 */
static inline void base_write_buffer2log(pcm_storeset_t *set, m_phlog_base_t *log) {
    /*
 ...
 * m_phlog_flush.
 */
static inline m_result_t m_phlog_base_write(pcm_storeset_t *set, m_phlog_base_t *log,
                                            pcm_word_t value) {
    /*
 ...
 *
 */
static inline m_result_t m_phlog_base_flush(pcm_storeset_t *set, m_phlog_base_t *log) {
    if (log->buffer_count > 0) { /* freud : There are still some stores left in buffer */
        base_write_buffer2log(set, log);
    }
    PCM_SEQSTREAM_FLUSH(set); /* freud : necessary fence */
    PCM_NT_STORE(set, (volatile pcm_word_t *)&log->nvmd->tail, (pcm_word_t)log->tail);
 ...
 *
 */
static inline m_result_t m_phlog_base_read(m_phlog_base_t *log, uint64_t *valuep) {
    uint64_t value;
 ...
 *
 */
static inline bool m_phlog_base_stable_exists(m_phlog_base_t *log) {
    if (log->read_index != log->nvmd->tail) {
        return true;
    }
    return false;
 ...
 *
 */
static inline void m_phlog_base_next_chunk(m_phlog_base_t *log) {
    uint64_t read_index;
 ...
 */
static inline m_result_t m_phlog_base_checkpoint_readindex(m_phlog_base_t *log,
                                                           uint64_t *readindex) {
 ...
 */
static inline void m_phlog_base_restore_readindex(m_phlog_base_t *log, uint64_t readindex) {
    log->read_index = readindex;
}

static inline m_result_t m_phlog_base_truncate_sync(pcm_storeset_t *set, m_phlog_base_t *phlog) {
...
m_result_t m_phlog_base_format(pcm_storeset_t *set, m_phlog_base_nvmd_t *nvmd, pcm_word_t *nvphlog,
                               int type);
...
m_result_t m_phlog_base_check_consistency(m_phlog_base_nvmd_t *nvmd, pcm_word_t *nvphlog,
