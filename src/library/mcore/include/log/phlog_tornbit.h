...
struct m_phlog_tornbit_nvmd_s {
    ...
    pcm_word_t
        flags; /**< the MSB part is reserved for flags while the LSB part stores the head index */
...
struct m_phlog_tornbit_s {
    uint64_t buffer[CHUNK_SIZE / sizeof(uint64_t)]; /**< software buffer to collect log writes till
                                                       we form a complete chunk */
    ...
    uint64_t write_remainder_nbits; /**< number of the valid least-significant bits of the
                                       write_remainder buffer */
    uint64_t read_remainder;        /**< the remainder bits after the decoding operation */
    uint64_t read_remainder_nbits;  /**< number of the valid least-significant bits of the
                                       read_remainder buffer */
    ...
    /* statistics */
    uint64_t
        pad1[8]; /**< some padding to avoid having statistics in the same cacheline with metadata */
...
static __attribute__((unused)) void print_binary64(uint64_t val) {
    int i;
 ...
 */
static inline void tornbit_write_buffer2log(pcm_storeset_t *set, m_phlog_tornbit_t *log) {
    /*
 ...
 * m_phlog_flush.
 */
static inline m_result_t m_phlog_tornbit_write(pcm_storeset_t *set, m_phlog_tornbit_t *log,
                                               pcm_word_t value) {
    /*
    ...
    if (log->buffer_count + 1 > CHUNK_SIZE / sizeof(pcm_word_t) - 1) {
        ...
        if (((log->tail + CHUNK_SIZE / sizeof(pcm_word_t)) & (PHYSICAL_LOG_NUM_ENTRIES - 1)) ==
...
#endif
            log->buffer[log->buffer_count] =
                ~TORN_MASK & (log->write_remainder | (value << log->write_remainder_nbits));
            ...
            log->write_remainder_nbits =
                (log->write_remainder_nbits + 1) &
                (64 - 1); /* efficient form of (log->write_remainder_nbits + 1) % 64 */
            ...
            if (log->write_remainder_nbits == 0) {
                log->buffer[0] = ~TORN_MASK & (log->write_remainder);
                log->buffer_count++;
 ...
 *
 */
static inline m_result_t m_phlog_tornbit_flush(pcm_storeset_t *set, m_phlog_tornbit_t *log) {
#ifdef _DEBUG_THIS
    ...
    if (log->write_remainder_nbits > 0) {
        ...
        if (((log->tail + CHUNK_SIZE / sizeof(pcm_word_t)) & (PHYSICAL_LOG_NUM_ENTRIES - 1)) ==
             ...
             * slot should suffice.
             */
            log->buffer[log->buffer_count] = ~TORN_MASK & (log->write_remainder);
            log->buffer_count++;
...
static inline m_result_t m_phlog_tornbit_read(m_phlog_tornbit_t *log, uint64_t *valuep) {
    uint64_t value;
    uint64_t tmp;

#ifdef _DEBUG_THIS
    printf("log_read: %lu %lu %lu\n", log->read_index, (uint64_t)log->read_remainder_nbits,
           log->stable_tail);
#endif
    /* Are there any stable data to read? */
    if (log->read_index != log->stable_tail && log->read_index + 1 != log->stable_tail) {
#ifdef _DEBUG_THIS
        printf("[%04lu]: 0x%016llX\n", log->read_index, ~TORN_MASK & log->nvphlog[log->read_index]);
#endif
 ...
 *
 */
static inline bool m_phlog_tornbit_stable_exists(m_phlog_tornbit_t *log) {
    if (log->read_index != log->stable_tail) {
        return true;
    }
    return false;
 ...
 *
 */
static inline void m_phlog_tornbit_next_chunk(m_phlog_tornbit_t *log) {
    uint64_t read_index;
    ...
    if (log->read_remainder_nbits > 0) {
        ...
        log->read_index =
            (read_index + CHUNK_SIZE / sizeof(pcm_word_t)) & (PHYSICAL_LOG_NUM_ENTRIES - 1);
    } else {
        log->read_remainder_nbits = 0;
        read_index = log->read_index & ~(CHUNK_SIZE / sizeof(pcm_word_t) - 1);
        /*
 ...
 */
static inline m_result_t m_phlog_tornbit_checkpoint_readindex(m_phlog_tornbit_t *log,
                                                              uint64_t *readindex) {
    if (log->read_remainder_nbits > 0) {
        return M_R_FAILURE;
    }
 ...
 */
static inline void m_phlog_tornbit_restore_readindex(m_phlog_tornbit_t *log, uint64_t readindex) {
    log->read_index = readindex;
    log->read_remainder_nbits = 0;
}

static inline m_result_t m_phlog_tornbit_truncate_sync(pcm_storeset_t *set,
                                                       m_phlog_tornbit_t *phlog) {
    phlog->head = phlog->tail;

    // FIXME: do we need a flush? PCM_NT_FLUSH(set);
    PCM_NT_STORE(set, (volatile pcm_word_t *)&phlog->nvmd->flags,
                 (pcm_word_t)(phlog->head | phlog->tornbit));
    PCM_NT_FLUSH(set);

    return M_R_SUCCESS;
}

m_result_t m_phlog_tornbit_format(pcm_storeset_t *set, m_phlog_tornbit_nvmd_t *nvmd,
                                  pcm_word_t *nvphlog, int type);
m_result_t m_phlog_tornbit_alloc(m_phlog_tornbit_t **phlog_tornbitp);
m_result_t m_phlog_tornbit_init(m_phlog_tornbit_t *phlog, m_phlog_tornbit_nvmd_t *nvmd,
                                pcm_word_t *nvphlog);
m_result_t m_phlog_tornbit_check_consistency(m_phlog_tornbit_nvmd_t *nvmd, pcm_word_t *nvphlog,
