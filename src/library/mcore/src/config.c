...
static void config_init_internal(char *config_file) {
    config_init(&mcore_cfg);
    config_read_file(&mcore_cfg, config_file);
    FOREACH_RUNTIME_CONFIG_SETTING(CONFIG_SETTING_LOOKUP, mcore, &mcore_cfg,
                                   &mcore_runtime_settings);
}
...
void mcore_config_fini() {

    config_destroy(&mcore_cfg);
}

void mcore_config_print(FILE *stream) {
