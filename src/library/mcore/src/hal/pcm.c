...
#define NO_PARTIAL_CRASH                                                                           \
    {                                                                                              \
        .word = { [CACHELINE_SIZE / sizeof(pcm_word_t)] = 1000000 }                                \
    }
...
static inline cacheline_t *cacheline_alloc() {
    cacheline_t *l;
...
static inline void cacheline_free(cacheline_t *l) {
    free(l);
}
...
void pcm_check_crash(pcm_storeset_t *set) {
...
static inline void crash_save_oldvalue(pcm_storeset_t *set, volatile pcm_word_t *addr) {
    uintptr_t byte_addr;
...
static inline void crash_restore_unflushed_values(pcm_storeset_t *set) {
    uintptr_t byte_addr;
    uintptr_t block_byte_addr;
    size_t i;
    int j;
 ...
 */
void crash_flush_cacheline(pcm_storeset_t *set, volatile pcm_word_t *addr,
                           int allow_partial_crash) {
    uintptr_t byte_addr;
    uintptr_t block_byte_addr;
    int random_number;
    size_t i;
...
flush_cacheline:
    ...
    if (successfully_flushed_words_num == CACHELINE_SIZE / sizeof(pcm_word_t)) {
        ...
        //            << (successfully_flushed_words_num * sizeof(pcm_word_t))) & (~(1ULL <<
        //            CACHELINE_SIZE - 1));
        bitmask = (((uint64_t)-1) << (successfully_flushed_words_num * sizeof(pcm_word_t))) &
                  ((uint64_t)-1);
    }
 ...
 */
static inline void crash_flush_cachelines(pcm_storeset_t *set, int all,
                                          int likelihood_flush_cacheline) {
    size_t i;
    size_t count;
    int random_number;
    int do_flush;
...
void pcm_trigger_crash(pcm_storeset_t *set, int wait_storesets_halt) {
    uint32_t waiters;
 ...
 */
static inline void nt_flush_buffers(pcm_storeset_t *set) {
    size_t i;
    size_t count;
