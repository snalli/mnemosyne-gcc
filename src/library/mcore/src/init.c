...
void do_global_init(void) {
    ...
    if (debug_fd != -1) {
        ret = write(debug_fd, "pmfs_mount", 10);
    } else {
        ret = -3;
        goto fail;
    }
...
fail:
    ...
    if (!mnemosyne_initialized) {
        ...
        op_time = 1000000 * (stop_time.tv_sec - start_time.tv_sec) + stop_time.tv_usec -
                  start_time.tv_usec;
...
void mnemosyne_fini_global(void) {
    do_global_fini();
}

mnemosyne_thrdesc_t *mnemosyne_init_thread(void) {
...
void mnemosyne_fini_thread(void) {
