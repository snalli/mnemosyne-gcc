...
static volatile char logmgr_initialized =
    0; /* reads and writes to single-byte memory locations are guaranteed to be atomic. Don't need
          to bother with alignment. */
 ...
 */
static m_log_ops_t static_log_ops[LF_TYPE_VALIDVALUES] = {NULL_LOG_OPS};
 ...
 */
static m_result_t create_log_pool(pcm_storeset_t *set, m_logmgr_t *mgr) {
    ...
    if (!log_pool) {
         ...
         */
        if (m_segment_find_using_addr((void *)LOG_POOL_START, &segidx_entry) != M_R_SUCCESS) {
            addr =
                m_pmap2((void *)LOG_POOL_START, LOG_POOL_SIZE, PROT_READ | PROT_WRITE, MAP_FIXED);
            if (addr == MAP_FAILED) {
    ...
    for (i = 0; i < LOG_NUM; i++) {
        log_dscs[i].nvmd = (m_log_nvmd_t *)(metadata_start_addr + sizeof(m_log_nvmd_t) * i);
        log_dscs[i].nvphlog = (pcm_word_t *)(logs_start_addr + physical_log_size * i);
        log_dscs[i].log = NULL;
        log_dscs[i].ops = NULL;
        log_dscs[i].logorder = INV_LOG_ORDER;
        if ((log_dscs[i].nvmd->generic_flags & LF_TYPE_MASK) == LF_TYPE_FREE) {
 ...
 */
static m_result_t logmgr_init(pcm_storeset_t *set) {
...
static m_result_t register_logtype(m_logmgr_t *mgr, int type, m_log_ops_t *ops, int lock) {
    m_result_t rv = M_R_FAILURE;
...
static m_result_t do_recovery(pcm_storeset_t *set, m_logmgr_t *mgr) {
    ...
    gettimeofday(&stop_time, NULL);
    op_time =
        1000000 * (stop_time.tv_sec - start_time.tv_sec) + stop_time.tv_usec - start_time.tv_usec;
 ...
 */
m_result_t m_logmgr_alloc_log(pcm_storeset_t *set, int type, uint64_t flags,
                              m_log_dsc_t **log_dscp) {
    m_result_t rv = M_R_FAILURE;
    m_log_dsc_t *log_dsc;
