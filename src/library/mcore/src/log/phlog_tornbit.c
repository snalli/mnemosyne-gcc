...
uint64_t load_nt_word(void *addr) {
    /* Portable 16-byte aligned load — replaces _mm_stream_load_si128 */
    char b[16];
    uintptr_t aladdr = ((uintptr_t)addr) & ~(uintptr_t)0xF;
    uintptr_t offset = ((uintptr_t)addr) & (uintptr_t)0xF;

    __atomic_thread_fence(__ATOMIC_ACQUIRE);
    memcpy(b, (void *)aladdr, 16);
    __atomic_thread_fence(__ATOMIC_ACQUIRE);

    uint64_t result;
    memcpy(&result, &b[offset], sizeof(result));
    return result;
}
...
static m_result_t tornbit_format_nvlog(pcm_storeset_t *set, uint32_t head_index, uint64_t tornbit,
                                       m_phlog_tornbit_nvmd_t *nvmd, pcm_word_t *nvphlog) {
    uint32_t i;
...
m_result_t m_phlog_tornbit_init(m_phlog_tornbit_t *phlog, m_phlog_tornbit_nvmd_t *nvmd,
    ...
    phlog->head = phlog->tail = phlog->stable_tail = phlog->read_index =
        phlog->nvmd->flags & LF_HEAD_MASK;
