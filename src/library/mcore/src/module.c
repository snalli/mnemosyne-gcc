...
static m_result_t persistent_shdr_open(char *module_path, uint64_t module_inode,
                                       uintptr_t module_start, module_dsr_t *module_dsr);
 ...
 */
void m_module_relocate_symbols(uintptr_t got_start, uintptr_t got_end, uintptr_t old_start,
                               uintptr_t old_end, uintptr_t new_start, uintptr_t new_end) {
    uintptr_t entry;
    ...
    mprotect((void *)(got_start - (got_start % 4096)), got_end - (got_start - (got_start % 4096)),
...
static __attribute__((unused)) void persistent_shdr_close(module_dsr_t *module_dsr) {
...
m_result_t m_module_create_module_dsr_list(struct list_head *module_dsr_listp) {
    ...
    while (!feof(f)) {
        if (fgets(buf, sizeof(buf), f) == 0)
            break;
        mapname[0] = '\0';
        sscanf(buf, "%lx-%lx %4s %lx %5s %lu %s", &start, &end, perm, &foo, dev, &inode, mapname);
        if (module_dsr == NULL) {
        ...
        if (strlen(mapname) > 0 && strcmp(prev_mapname, mapname) != 0) {
            if (persistent_shdr_open(mapname, inode, start, module_dsr) == M_R_SUCCESS) {
