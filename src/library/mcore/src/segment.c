...
static m_result_t segidx_find_entry_using_index(m_segidx_t *segidx, uint32_t index,
                                                m_segidx_entry_t **entryp);
 ...
 */
static void verify_backing_stores(m_segtbl_t *segtbl) {
    int n;
    DIR *d;
    struct dirent *dir;
    uint32_t index;
        ...
        while ((dir = readdir(d)) != NULL) {
            ...
            if (n == 2) {
                ...
                if (!(segtbl->entries[index].flags & SGTB_VALID_ENTRY)) {
                    ...
                    snprintf(complete_path, sizeof(complete_path), "%s/%s", SEGMENTS_DIR,
                             dir->d_name);
                ...
                if (segment_module_id != (uint64_t)(-1LLU)) {
                    if (segidx_find_entry_using_index(segtbl->idx, index, &ientry) != M_R_SUCCESS) {
...
static int create_backing_store(char *file, unsigned long long size) {
    int fd;
...
m_result_t m_check_backing_store(char *path, ssize_t size) {
...
static m_result_t segidx_insert_entry_ordered(m_segidx_t *segidx, m_segidx_entry_t *new_entry,
                                              int lock) {
    m_segidx_entry_t *ientry;
...
m_result_t segidx_create(m_segtbl_t *_segtbl, m_segidx_t **_segidxp) {
    m_result_t rv;
    m_segidx_t *_segidx;
    ...
    if (!(entries =
              (m_segidx_entry_t *)calloc(SEGMENT_TABLE_NUM_ENTRIES, sizeof(m_segidx_entry_t)))) {
        rv = M_R_NOMEMORY;
        goto err_calloc;
    }
...
static __attribute__((unused)) m_result_t segidx_destroy(m_segidx_t *segidx) {
    M_INTERNALERROR("Unimplemented functionality: segidx_destroy\n");
    return M_R_SUCCESS;
}

static m_result_t segidx_alloc_entry(m_segidx_t *segidx, m_segidx_entry_t **entryp) {
    m_result_t rv = M_R_FAILURE;
...
static m_result_t segidx_free_entry(m_segidx_t *segidx, m_segidx_entry_t *entry) {
    m_result_t rv = M_R_FAILURE;
...
/* Not thread safe */
static m_result_t segidx_find_entry_using_addr(m_segidx_t *segidx, void *addr,
                                               m_segidx_entry_t **entryp) {
    m_segidx_entry_t *ientry;
...
/* Not thread safe */
static m_result_t segidx_find_entry_using_module_id(m_segidx_t *segidx, uint64_t module_id,
                                                    m_segidx_entry_t **entryp) {
    m_segidx_entry_t *ientry;
...
static uintptr_t segidx_find_free_region(m_segidx_t *segidx, uintptr_t start_addr, size_t length) {
...
static inline m_result_t segment_table_map(m_segtbl_t *segtbl) {
    ...
    segtbl->entries = segment_map((void *)SEGMENT_TABLE_START, SEGMENT_TABLE_SIZE,
                                  PROT_READ | PROT_WRITE, MAP_PERSISTENT | MAP_SHARED, segtbl_fd);
 ...
 */
static m_result_t segment_table_incarnate() {
    m_result_t rv;
...
static inline void segment_table_print(m_segtbl_t *segtbl) {
    m_segidx_entry_t *ientry;
    m_segtbl_entry_t *tentry;
    uintptr_t start;
    uintptr_t end;
    ...
    list_for_each_entry(ientry, &segtbl->idx->mapped_entries.list, list) {
        ...
        M_DEBUG_PRINT(M_DEBUG_SEGMENT, "0x%016lx - 0x%016lx %16luK", start, end,
                      (long unsigned int)tentry->size / 1024);
...
void m_segment_table_print() {
    segment_table_print(&m_segtbl);
}

m_result_t m_segment_table_alloc_entry(m_segtbl_t *segtbl, m_segtbl_entry_t **entryp) {
    m_result_t rv;
    m_segidx_entry_t *ientry;
...
static inline void *segment_map(void *addr, size_t size, int prot, int flags, int segment_fd) {
    ...
    segmentp = mmap(addr, size, prot, flags | MAP_PERSISTENT | MAP_SHARED, segment_fd, 0);
    ...
    if (start < PSEGMENT_RESERVED_REGION_START || start > PSEGMENT_RESERVED_REGION_END ||
 ...
 */
static void *segment_map2(void *addr, size_t size, int prot, int flags, char *segment_path) {
    int segment_fd;
    void *segmentp;
 ...
 */
void segment_reincarnate_segments(m_segtbl_t *segtbl) {
    m_segidx_entry_t *ientry;
    m_segtbl_entry_t *tentry;
    uintptr_t start;
    uintptr_t end;
    ...
    list_for_each_entry(ientry, &segtbl->idx->mapped_entries.list, list) {
        ...
        if (tentry->flags & SGTB_TYPE_PMAP) {
            ...
            sprintf(path, "%s/%d.%lu", SEGMENTS_DIR, ientry->index,
                    (long unsigned int)ientry->module_id);
        } else {
        ...
        map_addr = segment_map2((void *)start, (size_t)tentry->size, PROT_READ | PROT_WRITE,
                                MAP_FIXED, path);
...
static void *pmap_internal_abs(void *start, unsigned long long length, int prot, int flags,
                               m_segidx_entry_t **entryp, uint32_t segtbl_entry_flags,
                               uint64_t module_id) {
    char path[256];
...
static void *pmap_internal(void *start, unsigned long long length, int prot, int flags,
                           m_segidx_entry_t **entryp, uint32_t segtbl_entry_flags,
                           uint64_t module_id) {
    uintptr_t start_addr = SEGMENT_MAP_START + (uintptr_t)start;

    segment_table_print(&m_segtbl);
    return pmap_internal_abs((void *)start_addr, length, prot, flags, entryp, segtbl_entry_flags,
                             module_id);
}
 ...
 *
 */
static m_result_t segment_create_sections(m_segtbl_t *segtbl) {
    void *mapped_addr;
    size_t length;
    ...
    list_for_each_entry(module_dsr, &module_dsr_list, list) {
        ...
        if (segidx_find_entry_using_module_id(segtbl->idx, module_dsr->module_inode, &ientry) !=
            ...
            mapped_addr =
                pmap_internal(0, length, PROT_READ | PROT_WRITE, 0, &ientry,
        ...
        persistent_section_absolute_addr =
            (uintptr_t)(module_dsr->persistent_shdr.sh_addr + module_dsr->module_start - 0x400000);
        GOT_section_absolute_addr =
            (uintptr_t)(module_dsr->GOT_shdr.sh_addr + module_dsr->module_start - 0x400000);

        M_DEBUG_PRINT(M_DEBUG_SEGMENT, "persistent_section.start         = %p\n",
                      (void *)persistent_section_absolute_addr);
        M_DEBUG_PRINT(
            M_DEBUG_SEGMENT, "persistent_section.end           = %p\n",
            (void *)(persistent_section_absolute_addr + module_dsr->persistent_shdr.sh_size));
        ...
        m_module_relocate_symbols(
            GOT_section_absolute_addr, GOT_section_absolute_addr + module_dsr->GOT_shdr.sh_size,
            persistent_section_absolute_addr,
            persistent_section_absolute_addr + module_dsr->persistent_shdr.sh_size,
            (uintptr_t)mapped_addr, (uintptr_t)mapped_addr + length);
