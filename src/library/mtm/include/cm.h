...
static inline int cm_conflict(mtm_tx_t *tx, volatile mtm_word_t *lock, mtm_word_t *l) {
    ...
    if (tx->retries >= cm_threshold) {
        if (LOCK_GET_PRIORITY(*l) < tx->priority ||
            (LOCK_GET_PRIORITY(*l) == tx->priority && *l < (mtm_word_t)modedata->w_set.entries &&
             !LOCK_GET_WAIT(*l))) {
...
static inline int cm_upgrade_lock(mtm_tx_t *tx) {
#if CM == CM_PRIORITY
...
static inline void cm_delay(mtm_tx_t *tx) {
#if CM == CM_BACKOFF
    unsigned long wait;
    volatile int j;
...
static inline void cm_visible_read(mtm_tx_t *tx) {
#if CM == CM_PRIORITY
    tx->visible_reads++;
#endif /* CM == CM_PRIORITY */
}

static inline void cm_reset(mtm_tx_t *tx) {
