...
#define DEFINE_LOAD_BYTES(NAME) DEFINE_TM_LOAD_BYTES(NAME)

#define DEFINE_STORE_BYTES(NAME) DEFINE_TM_STORE_BYTES(NAME)

#define DEFINE_TM_LOAD_BYTES(NAME)                                                                 \
    void mtm_##NAME##_load_bytes(mtm_tx_t *tx, volatile uint8_t *addr, uint8_t *buf,               \
                                 size_t size) {                                                    \
        convert_t val;                                                                             \
        unsigned int i;                                                                            \
        mtm_word_t *a;                                                                             \
...
#define DEFINE_TM_STORE_BYTES(NAME)                                                                \
    void mtm_##NAME##_store_bytes(mtm_tx_t *tx, volatile uint8_t *addr, uint8_t *buf,              \
                                  size_t size) {                                                   \
        convert_t val;                                                                             \
        convert_t mask;                                                                            \
        unsigned int i;                                                                            \
...
#define READ_BARRIER(NAME, T, LOCK)                                                                \
    _ITM_TYPE_##T _ITM_CALL_CONVENTION _ITM_##LOCK##T(const _ITM_TYPE_##T *addr) {                 \
        mtm_tx_t *tx = mtm_get_tx();                                                               \
        _ITM_TYPE_##T val;                                                                         \
        mtm_##NAME##_load_bytes(tx, (volatile uint8_t *)addr, (uint8_t *)&val,                     \
                                sizeof(_ITM_TYPE_##T));                                            \
        return val;                                                                                \
    }

#define WRITE_BARRIER(NAME, T, LOCK)                                                               \
    void _ITM_CALL_CONVENTION _ITM_##LOCK##T(const _ITM_TYPE_##T *addr, _ITM_TYPE_##T value) {     \
        mtm_tx_t *tx = mtm_get_tx();                                                               \
        mtm_##NAME##_store_bytes(tx, (volatile uint8_t *)addr, (uint8_t *)&value,                  \
                                 sizeof(_ITM_TYPE_##T));                                           \
    }
