 ...
 */
static inline r_entry_t *mtm_has_read(mtm_tx_t *tx, mode_data_t *modedata,
                                      volatile mtm_word_t *lock) {
    r_entry_t *r;
    int i;

    PRINT_DEBUG("==> mtm_has_read(%p[%lu-%lu],%p)\n", tx, (unsigned long)modedata->start,
                (unsigned long)modedata->end, lock);
 ...
 */
static inline int mtm_validate(mtm_tx_t *tx, mode_data_t *modedata) {
    r_entry_t *r;
    int i;
    mtm_word_t l;
 ...
 */
static inline void mtm_allocate_rs_entries(mtm_tx_t *tx, mode_data_t *data, int extend) {
    if (extend) {
        /* Extend read set */
        data->r_set.size *= 2;
        PRINT_DEBUG2("==> reallocate read set (%p[%lu-%lu],%d)\n", tx, (unsigned long)data->start,
                     (unsigned long)data->end, data->r_set.size);
        if ((data->r_set.entries = (r_entry_t *)realloc(
                 data->r_set.entries, data->r_set.size * sizeof(r_entry_t))) == NULL) {
            perror("realloc");
            exit(1);
        }
    } else {
        /* Allocate read set */
        if ((data->r_set.entries = (r_entry_t *)malloc(data->r_set.size * sizeof(r_entry_t))) ==
            NULL) {
            perror("malloc");
            exit(1);
        }
 ...
 */
static inline void mtm_allocate_ws_entries(mtm_tx_t *tx, mode_data_t *data, int extend) {
    ...
    if (extend) {
        ...
        PRINT_DEBUG("==> reallocate write set (%p[%lu-%lu],%d)\n", tx, (unsigned long)data->start,
                    (unsigned long)data->end, data->w_set.size);
        if ((data->w_set.entries = (w_entry_t *)realloc(
                 data->w_set.entries, data->w_set.size * sizeof(w_entry_t))) == NULL) {
            perror("realloc");
            exit(1);
        }
    } else {
        ...
        if ((data->w_set.entries = (w_entry_t *)malloc(data->w_set.size * sizeof(w_entry_t))) ==
            NULL) {
            perror("malloc");
            exit(1);
        }
#else
        if (posix_memalign((void **)&data->w_set.entries, ALIGNMENT,
