...
#define FOREACH_ABI_FUNCTION(ACTION, ...)                                                          \
    ACTION(void, abortTransaction, (_ITM_abortReason reason, const _ITM_srcLocation *loc),         \
           __VA_ARGS__)                                                                            \
    ACTION(void, rollbackTransaction, (const _ITM_srcLocation *loc), __VA_ARGS__)                  \
    ACTION(void, commitTransaction, (const _ITM_srcLocation *loc), __VA_ARGS__)                    \
    ACTION(bool, tryCommitTransaction, (const _ITM_srcLocation *loc), __VA_ARGS__)                 \
    ACTION(void, commitTransactionToId,                                                            \
           (const _ITM_transactionId tid, const _ITM_srcLocation *loc), __VA_ARGS__)               \
    ACTION(uint32_t, beginTransaction, (uint32 __properties, const _ITM_srcLocation *loc),         \
           __VA_ARGS__)
