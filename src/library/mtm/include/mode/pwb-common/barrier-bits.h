 ...
 */
static inline int pwb_extend(mtm_tx_t *tx, mode_data_t *modedata) {
    mtm_word_t now;
 ...
 */
static w_entry_t *initialize_write_set_entry(w_entry_t *entry, volatile mtm_word_t *address,
                                             mtm_word_t value, mtm_word_t mask,
                                             volatile mtm_word_t version, volatile mtm_word_t *lock,
                                             int is_nonvolatile) {
 ...
 */
static void insert_write_set_entry_after(w_entry_t *new_entry, w_entry_t *tail,
                                         mtm_tx_t *transaction, w_entry_t *cache_neighbor) {
    ...
    if (new_entry->is_nonvolatile) {
        M_TMLOG_WRITE(transaction->pcm_storeset, modedata->ptmlog, (uintptr_t)new_entry->addr,
                      new_entry->value, new_entry->mask);
    }
}
 ...
 */
static inline w_entry_t *matching_write_set_entry(w_entry_t *const list_head,
 ...
 */
static inline w_entry_t *pwb_write_internal(mtm_tx_t *tx, volatile mtm_word_t *addr,
                                            mtm_word_t value, mtm_word_t mask,
    ...
    MTM_DEBUG_PRINT("==> pwb_write(t=%p[%lu-%lu],a=%p,d=%llX-%llu,m=0x%llX)\n", tx,
                    (unsigned long)modedata->start, (unsigned long)modedata->end, addr,
                    (unsigned long long)value, (unsigned long long)value, (unsigned long long)mask);
    /* Check status */
    // assert(tx->status == TX_ACTIVE);
...
restart_no_load:
    if (LOCK_GET_OWNED(l)) {
        ...
        if (modedata->w_set.entries <= write_set_head &&
            ...
            w_entry_t *matching_entry = matching_write_set_entry(
                write_set_head, addr, &write_set_tail, &last_entry_in_same_cache_block);
            if (matching_entry != NULL) {
                if (matching_entry->mask != 0) {
                    mask_new_value(matching_entry, addr, value, mask);
                    /* Write out the entry to the persistent TM log? */
                    if (access_is_nonvolatile) {
                        M_TMLOG_WRITE(tx->pcm_storeset, modedata->ptmlog,
                                      (uintptr_t)matching_entry->addr, matching_entry->value,
                                      matching_entry->mask);
                    }
                }
                ...
                if (modedata->w_set.nb_entries == modedata->w_set.size) {
                    /* Extend write set (invalidate pointers to write set entries => abort and
                     * reallocate) */
                    ...
                    if (access_is_nonvolatile) {
                        m_stats_statset_increment(mtm_statsmgr, tx->statset, XACT,
                                                  nvwrites_distinct, 1);
                    } else {
                        m_stats_statset_increment(mtm_statsmgr, tx->statset, XACT, vwrites_distinct,
                                                  1);
                    }
#endif
                    ...
                    version =
                        write_set_tail->version; // Get version from previous write set entry (all
                                                 // entries in linked list have same version)
                    w_entry_t *initialized_entry = initialize_write_set_entry(
                        w, addr, value, mask, version, lock, access_is_nonvolatile);

                    // Add entry to the write set
                    insert_write_set_entry_after(initialized_entry, write_set_tail, tx,
                                                 last_entry_in_same_cache_block);
        ...
        w_entry_t *initialized_entry =
            initialize_write_set_entry(w, addr, value, mask, version, lock, access_is_nonvolatile);
...
static inline mtm_word_t pwb_load_internal(mtm_tx_t *tx, volatile mtm_word_t *addr,
                                           int enable_isolation) {
    assert(tx->mode == MTM_MODE_pwbnl || tx->mode == MTM_MODE_pwbetl);
    mode_data_t *modedata = (mode_data_t *)tx->modedata[tx->mode];
    volatile mtm_word_t *lock;
...
restart_no_load:
    if (LOCK_GET_OWNED(l)) {
        ...
        if (modedata->w_set.entries <= w &&
            ...
            MTM_DEBUG_PRINT("(t=%p[%lu-%lu],a=%p,l=%p,*l=%lu,d=%p-%lu)\n", tx,
                            (unsigned long)modedata->start, (unsigned long)modedata->end, addr,
                            lock, (unsigned long)l, (void *)value, (unsigned long)value);
            return value;
        }
        /* If isolation is off and the pseudo-lock was set then we should have already
         * found a written-back value entry and never reach here. */
    ...
    MTM_DEBUG_PRINT("==> mtm_pwb_load(t=%p[%lu-%lu],a=%p,l=%p,*l=%lu,d=%p-%lu,v=%lu)\n", tx,
                    (unsigned long)modedata->start, (unsigned long)modedata->end, addr, lock,
                    (unsigned long)l, (void *)value, (unsigned long)value, (unsigned long)version);
