...
static inline bool pwb_trycommit(mtm_tx_t *tx, int enable_isolation) {
    ...
    PRINT_DEBUG("==> mtm_commit(%p[%lu-%lu] nest_level:%d-->%d)\n", tx,
                (unsigned long)modedata->start, (unsigned long)modedata->end, tx->nesting,
                tx->nesting - 1);
    ...
    if (modedata->w_set.nb_entries > 0) {
        ...
        /* Make sure previous stores are not reordered with the cl-flushes below  freud :
         * unnecessary fence */
        /* PCM_WB_FENCE(tx->pcm_storeset);  moved this info M_TMLOG_COMMIT. It replaces PCM_NT_FLUSH
         * in m_tmlog_base_? */
        ...
        for (i = modedata->w_set.nb_entries; i > 0; i--, w++) {
            MTM_DEBUG_PRINT("==> write(t=%p[%lu-%lu],a=%p,d=%p-%d,m=%llx,v=%d)\n", tx,
                            (unsigned long)modedata->start, (unsigned long)modedata->end, w->addr,
                            (void *)w->value, (int)w->value, (unsigned long long)w->mask,
                            (int)w->version);
            /* Write the value in this entry to memory (it will probably land in the cache; that's
             * okay.) */
            if (w->mask != 0) {
                PCM_WB_STORE_ALIGNED_MASKED(tx->pcm_storeset, w->addr, w->value, w->mask);
            }
#ifdef SYNC_TRUNCATION
            /* Flush the cacheline to persistent memory if this is the last entry in this cache
             * line. */
            if (w->next_cache_neighbor == NULL) {
                ...
                if (enable_isolation) {
                    if (((uintptr_t)w->addr >= PSEGMENT_RESERVED_REGION_START &&
                         (uintptr_t)w->addr <
                             (PSEGMENT_RESERVED_REGION_START + PSEGMENT_RESERVED_REGION_SIZE))) {
 ...
 */
static inline void pwb_rollback(mtm_tx_t *tx) {
    ...
    if (i > 0) {
        ...
        for (; i > 0; i--, w++) {
            ...
            PRINT_DEBUG2("==> discard(t=%p[%lu-%lu],a=%p,d=%p-%lu,v=%lu)\n", tx,
                         (unsigned long)modedata->start, (unsigned long)modedata->end, w->addr,
                         (void *)w->value, (unsigned long)w->value, (unsigned long)w->version);
        }
...
static void rollback_transaction(mtm_tx_t *tx) {
    pwb_rollback(tx);
    switch (tx->mode) {
    case MTM_MODE_pwbnl:
        assert(0);
...
static inline void pwb_prepare_transaction(mtm_tx_t *tx) {
    assert(tx->mode == MTM_MODE_pwbnl || MTM_MODE_pwbetl);
    mode_data_t *modedata = (mode_data_t *)tx->modedata[tx->mode];

start:
    /* Start timestamp */
    modedata->start = modedata->end =
        GET_CLOCK; /* OPT: Could be delayed until first read/write, why bother ? */
    /* Allow extensions */
    tx->can_extend = 1;
#ifdef ROLLOVER_CLOCK
...
static inline uint32_t beginTransaction_internal(mtm_tx_t *tx, uint32_t prop,
                                                 _ITM_srcLocation *srcloc, int enable_isolation,
                                                 jmp_buf **__env) {
    assert(tx->mode == MTM_MODE_pwbnl || MTM_MODE_pwbetl);
    ...
    *__env = &(tx->jb);
    ...
    if ((prop & pr_doesGoIrrevocable) || !(prop & pr_instrumentedCode)) {
        ...
        return (prop & pr_uninstrumentedCode ? a_runUninstrumentedCode : a_runInstrumentedCode);
    }
...
static __attribute__((unused)) bool trycommit_transaction(mtm_tx_t *tx, int enable_isolation) {
