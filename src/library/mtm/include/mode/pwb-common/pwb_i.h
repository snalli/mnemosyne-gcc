...
struct mtm_pwb_w_entry_s {
    union { /* For padding... */
        struct {
            ...
            struct mtm_pwb_w_entry_s
                *next_cache_neighbor; /* Next address covered by same lock and falls within the same
                                         cacheline. These entries can be written together with a
...
struct mtm_pwb_mode_data_s {
    ...
    M_TMLOG_T *ptmlog; /**< The persistent tm log; this is to avoid dereferencing ptmlog_dsc in the
                          fast path */
};
