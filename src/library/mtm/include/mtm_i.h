...
struct mtm_tx_s {
    ...
    jmp_buf *tmp_jb_ptr; /* Pointer to the temporary register checkpoint. This simplifies the
                            assembly checkpoint code. */
    jmp_buf tmp_jb; /* Temporary register checkpoint; this is where the assebly register checkpoint
                       code will save the registers.  */
    jmp_buf jb; /* Register checkpoint of this transaction. Keeping this separate from tmp_jb allows
                   a nested transaction to transparently execute the assembly checkpoint code
                   without destroying the active register checkpoint.  */
...
extern uint32_t mtm_longjmp(const mtm_jmpbuf_t *, uint32_t) ITM_NORETURN;
...
#ifdef LOCK_IDX_SWAP
/*
 * Compute index in lock table (swap bytes to avoid consecutive addresses to have neighboring
 * locks).
 */
