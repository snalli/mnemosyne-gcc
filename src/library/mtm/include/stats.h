 ...
 * exclusive.
 */
#define FOREACH_STAT(ACTION) FOREACH_STAT_XACT(ACTION)

#define FOREACH_STATPROBE(ACTION) ACTION(XACT)
...
struct m_stats_stat_s {
    m_stats_statcounter_t max;   /**< Maximum number of the statistic across all transactions of an
                                    atomic block (static transaction). */
    m_stats_statcounter_t min;   /**< Minimum number of the statistic across all transactions of an
                                    atomic block (static transaction). */
    m_stats_statcounter_t total; /**< Total number of the statistic across all transactions of an
                                    atomic block (static transaction). */
};
...
static inline void m_stats_statset_set_val(m_stats_statset_t *statset, m_stats_statentry_t entry,
                                           m_stats_statcounter_t val) {
    statset->stats[entry].total = val;
}

/** \brief Reads the value of a statistic. */
static inline m_stats_statcounter_t m_stats_statset_get_val(m_stats_statset_t *statset,
                                                            m_stats_statentry_t entry) {
    return statset->stats[entry].total;
}

#define GENERATE_STATPROBES(stat_provider)                                                         \
    static inline void m_stats_statset_increment_##stat_provider(                                  \
        m_statsmgr_t *statsmgr, m_stats_statset_t *statset, m_stats_statentry_t entry,             \
        m_stats_statcounter_t val) {                                                               \
        if (statsmgr) {                                                                            \
            m_stats_statset_set_val(statset, entry,                                                \
                                    m_stats_statset_get_val(statset, entry) + val);                \
        }                                                                                          \
    }                                                                                              \
                                                                                                   \
    static inline void m_stats_statset_decrement_##stat_provider(                                  \
        m_statsmgr_t *statsmgr, m_stats_statset_t *statset, m_stats_statentry_t entry,             \
        m_stats_statcounter_t val) {                                                               \
        if (statsmgr) {                                                                            \
            m_stats_statset_set_val(statset, entry,                                                \
...
#define m_stats_statset_increment(statsmgr, statset, stat_provider, stat, val)                     \
    m_stats_statset_increment_##stat_provider(statsmgr, statset, m_stats_##stat##_stat, val);

#define m_stats_statset_decrement(statsmgr, statset, stat_provider, stat, val)                     \
    m_stats_statset_decrement_##stat_provider(statsmgr, statset, m_stats_##stat##_stat, val);
