...
static void pmalloc_undo_wrapper(void *ptr) {
    mtm_pmalloc_undo((size_t)(uintptr_t)ptr);
}
...
extern void mtm_pwbetl_commitTransactionToId(mtm_tx_t *, _ITM_transactionId,
...
static void *find_clone(void *ptr) {
...
void *_ITM_CALL_CONVENTION _ITM_getTMCloneOrIrrevocable(void *ptr) {
...
void *_ITM_CALL_CONVENTION _ITM_getTMCloneSafe(void *ptr) {
    void *ret = find_clone(ptr);
    if (ret == NULL) {
...
static int clone_entry_compare(const void *a, const void *b) {
...
void _ITM_registerTMCloneTable(void *xent, size_t size) {
...
void _ITM_deregisterTMCloneTable(void *xent) {
    struct clone_entry *ent = (struct clone_entry *)(xent);
    ...
    for (pprev = &all_tables; tab = *pprev, tab->table != ent; pprev = &tab->next)
        continue;
    *pprev = tab->next;
...
void _ITM_CALL_CONVENTION ITM_NORETURN _ITM_error(const _ITM_srcLocation *loc UNUSED,
...
void _ITM_CALL_CONVENTION _ITM_dropReferences(const void *start, size_t size) {
    // TODO
    assert(0);
}

void _ITM_CALL_CONVENTION _ITM_registerThrownObject(const void *exception_object, size_t s) {
    assert(0);
}
...
void _ITM_CALL_CONVENTION _ITM_addUserCommitAction(_ITM_userCommitFunction fn,
                                                   _ITM_transactionId tid, void *arg) {
    mtm_tx_t *tx = mtm_get_tx();
    mtm_useraction_addUserCommitAction(tx, fn, tid, arg);
}

void _ITM_CALL_CONVENTION _ITM_addUserUndoAction(const _ITM_userUndoFunction fn, void *arg) {
    mtm_tx_t *tx = mtm_get_tx();
    mtm_useraction_addUserUndoAction(tx, fn, arg);
}

void _ITM_CALL_CONVENTION _ITM_changeTransactionMode(_ITM_transactionState __mode,
