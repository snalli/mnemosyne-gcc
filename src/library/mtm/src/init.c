 ...
 */
static __attribute__((unused)) void signal_catcher(int sig) {
...
#undef ACTION
#define ACTION(mode) mtm_##mode##_create(tx, &(tx->modedata[MTM_MODE_##mode]));
    FOREACH_MODE(ACTION)
#undef ACTION
    ...
    if ((tx->wb_table = (mtm_word_t *)calloc(PRIVATE_LOCK_ARRAY_SIZE, sizeof(mtm_word_t))) ==
        NULL) {
        perror("malloc");
        exit(1);
    }
...
#undef ACTION
#define ACTION(mode) mtm_##mode##_destroy((mtm_mode_data_t *)tx->modedata[MTM_MODE_##mode]);
    FOREACH_MODE(ACTION)
#undef ACTION
