...
static void local_allocate(mtm_tx_t *tx, int extend) {
    ...
    if (extend) {
        ...
        if ((local_undo->buf = (char *)realloc((void *)local_undo->buf, local_undo->size)) ==
            NULL) {
            perror("realloc");
            exit(1);
        }
    } else {
        if ((local_undo->buf = (char *)malloc(local_undo->size)) == NULL) {
            perror("malloc");
            exit(1);
        }
    }
...
void mtm_local_rollback(mtm_tx_t *tx) {
    ...
    while (local_undo_entry) {
        ...
        entryp =
            (uintptr_t)local_undo_entry - local_undo_entry->len - sizeof(mtm_local_undo_entry_t);
...
static void _ITM_CALL_CONVENTION log_arbitrarily(mtm_tx_t *tx, const volatile void *ptr,
                                                 size_t len) {
    mtm_local_undo_t *local_undo = &tx->local_undo;
    mtm_local_undo_entry_t *local_undo_entry;
    char *buf __attribute__((unused));
