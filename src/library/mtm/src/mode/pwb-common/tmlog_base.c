...
m_result_t m_tmlog_base_init(pcm_storeset_t *set, m_log_t *log, m_log_dsc_t *log_dsc) {
    ...
    m_phlog_base_format(set, (m_phlog_base_nvmd_t *)log_dsc->nvmd, log_dsc->nvphlog,
                        LF_TYPE_TM_BASE);
    m_phlog_base_init(phlog_base, (m_phlog_base_nvmd_t *)log_dsc->nvmd, log_dsc->nvphlog);
            ...
            if (m_phlog_base_read(&(tmlog->phlog_base), &addr) == M_R_SUCCESS) {
                if (addr == XACT_COMMIT_MARKER) {
                    ...
                    for (i = 0; i < ((PointerHash *)tmlog->flush_set)->size; i++) {
                        PointerHashRecord *r = PointerHashRecords_recordAt_(
                            ((PointerHash *)tmlog->flush_set)->records, i);
                        if (block_addr = (uintptr_t)r->k) {
                            PointerHash_removeKey_((PointerHash *)tmlog->flush_set,
                                                   (void *)block_addr);
                        }
                    }
#endif
...
retry:
    if (m_phlog_base_stable_exists(&(tmlog->phlog_base))) {
         ...
         * transaction.
         */
        assert(m_phlog_base_checkpoint_readindex(&(tmlog->phlog_base), &readindex_checkpoint) ==
               M_R_SUCCESS);
        while (1) {
            if (m_phlog_base_read(&(tmlog->phlog_base), &addr) == M_R_SUCCESS) {
                if (addr == XACT_COMMIT_MARKER) {
...
m_result_t m_tmlog_base_recovery_init(pcm_storeset_t *set, m_log_dsc_t *log_dsc) {
    ...
    m_phlog_base_init(&tmlog->phlog_base, (m_phlog_base_nvmd_t *)log_dsc->nvmd, log_dsc->nvphlog);
