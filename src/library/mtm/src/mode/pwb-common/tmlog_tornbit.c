...
m_result_t m_tmlog_tornbit_init(pcm_storeset_t *set, m_log_t *log, m_log_dsc_t *log_dsc) {
    ...
    m_phlog_tornbit_format(set, (m_phlog_tornbit_nvmd_t *)log_dsc->nvmd, log_dsc->nvphlog,
                           LF_TYPE_TM_TORNBIT);
    m_phlog_tornbit_init(phlog_tornbit, (m_phlog_tornbit_nvmd_t *)log_dsc->nvmd, log_dsc->nvphlog);

    return M_R_SUCCESS;
}

static inline m_result_t truncation_prepare(pcm_storeset_t *set, m_log_dsc_t *log_dsc) {
            ...
            if (m_phlog_tornbit_read(&(tmlog->phlog_tornbit), &addr) == M_R_SUCCESS) {
                if (addr == XACT_COMMIT_MARKER) {
                    ...
                    if (!PointerHash_at_((PointerHash *)tmlog->flush_set, (void *)block_addr)) {
                        PointerHash_at_put_((PointerHash *)tmlog->flush_set, (void *)block_addr,
                                            (void *)1);
                    }
#else
                ...
                M_INTERNALERROR(
                    "Invariant violation: there must be at least one atomic log fragment.");
            }
        }
...
m_result_t m_tmlog_tornbit_truncation_do(pcm_storeset_t *set, m_log_dsc_t *log_dsc) {
    ...
    for (i = 0; i < (int)((PointerHash *)tmlog->flush_set)->size; i++) {
        PointerHashRecord *r =
            PointerHashRecords_recordAt_(((PointerHash *)tmlog->flush_set)->records, i);
        if ((block_addr = (uintptr_t)r->k)) {
            // PointerHash_removeKey_noshrink((PointerHash *) tmlog->flush_set, (void *)
            // block_addr);
...
static inline m_result_t recovery_prepare_next(pcm_storeset_t *set, m_log_dsc_t *log_dsc) {
    m_tmlog_tornbit_t *tmlog = (m_tmlog_tornbit_t *)log_dsc->log;
    pcm_word_t value;
    uint64_t sqn = INV_LOG_ORDER;
    uintptr_t addr;
...
retry:
    if (m_phlog_tornbit_stable_exists(&(tmlog->phlog_tornbit))) {
         ...
         * transaction.
         */
        assert(m_phlog_tornbit_checkpoint_readindex(&(tmlog->phlog_tornbit),
                                                    &readindex_checkpoint) == M_R_SUCCESS);
        while (1) {
            if (m_phlog_tornbit_read(&(tmlog->phlog_tornbit), &addr) == M_R_SUCCESS) {
                if (addr == XACT_COMMIT_MARKER) {
                    assert(m_phlog_tornbit_read(&(tmlog->phlog_tornbit), &sqn) == M_R_SUCCESS);
                    m_phlog_tornbit_restore_readindex(&(tmlog->phlog_tornbit),
                                                      readindex_checkpoint);
                    break;
                } else if (addr == XACT_ABORT_MARKER) {
                    assert(m_phlog_tornbit_read(&(tmlog->phlog_tornbit), &sqn) == M_R_SUCCESS);
                    m_phlog_tornbit_next_chunk(&tmlog->phlog_tornbit);
...
m_result_t m_tmlog_tornbit_recovery_init(pcm_storeset_t *set, m_log_dsc_t *log_dsc) {
    ...
    m_phlog_tornbit_init(&tmlog->phlog_tornbit, (m_phlog_tornbit_nvmd_t *)log_dsc->nvmd,
                         log_dsc->nvphlog);

    m_phlog_tornbit_check_consistency((m_phlog_tornbit_nvmd_t *)log_dsc->nvmd, log_dsc->nvphlog,
                                      &(tmlog->phlog_tornbit.stable_tail));
