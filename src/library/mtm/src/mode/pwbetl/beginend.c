...
uint32_t mtm_pwbetl_beginTransaction_internal(mtm_tx_t *tx, uint32_t prop, _ITM_srcLocation *srcloc,
                                              jmp_buf **__env) {
    PM_START_TX();
    return beginTransaction_internal(tx, prop, srcloc, 1, __env);
}

void _ITM_CALL_CONVENTION mtm_pwbetl_rollbackTransaction(mtm_tx_t *tx,
...
void _ITM_CALL_CONVENTION mtm_pwbetl_abortTransaction(mtm_tx_t *tx, _ITM_abortReason reason,
                                                      const _ITM_srcLocation *loc) {
    assert(reason == userAbort || reason == userRetry);
    /* Compiler Bug: pr_hasNoRetry seems to be wrong */
    assert((reason == userAbort && (tx->prop & pr_hasNoAbort) == 0) || (reason == userRetry && 1));
...
bool _ITM_CALL_CONVENTION mtm_pwbetl_tryCommitTransaction(mtm_tx_t *tx,
                                                          const _ITM_srcLocation *loc) {
    return trycommit_transaction(tx, 1);
}

void _ITM_CALL_CONVENTION mtm_pwbetl_commitTransaction(mtm_tx_t *tx, const _ITM_srcLocation *loc) {
...
void _ITM_CALL_CONVENTION mtm_pwbetl_commitTransactionToId(mtm_tx_t *tx,
