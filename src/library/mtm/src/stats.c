...
char *stats_strings[] = {FOREACH_STAT(ACTION)};
#undef ACTION
...
m_result_t m_statsmgr_destroy(m_statsmgr_t **statsmgrp) {
    ...
    for (threadstat = statsmgr->alloc_threadstat_list_head; threadstat != NULL;
...
m_result_t m_stats_threadstat_create(m_statsmgr_t *statsmgr, unsigned int tid,
    ...
    threadstat->tid = tid;
    m_chhash_create(&threadstat->stats_table, M_STATS_THREADSTAT_HASHTABLE_SIZE, false);
...
static void stats_statset_reset(m_stats_statset_t *statset) {
...
static m_result_t stats_get_statset(m_chhash_t *stats_table, const char *name,
...
void stats_aggregate(m_stats_statset_t *dest_statset, m_stats_statset_t *source_statset) {
    ...
    for (i = 0; i < m_stats_numofstats; i++) {
        ...
        dest_statset->stats[i].min =
            MIN(dest_statset->stats[i].min, source_statset->stats[i].total);
        dest_statset->stats[i].max =
            MAX(dest_statset->stats[i].max, source_statset->stats[i].total);
    }
}

void m_stats_threadstat_aggregate(m_stats_threadstat_t *threadstat,
    ...
    if (result != M_R_SUCCESS) {
        ...
        m_chhash_add(threadstat->stats_table, (m_chhash_key_t)source_statset->name,
...
static void m_stats_statset_print(FILE *fout, m_stats_statset_t *statset, int shiftlen,
                                  int print_header) {
    int i __attribute__((unused));
    char header[512];
    double mean;
    ...
    fprintf(fout, "%s%s%s:%13s%13s%13s%13s\n", WHITESPACE(shiftlen + 2), "", WHITESPACE(25), "Min",
            "Mean", "Max", "Total");

    count = statset->count;

    fprintf(fout, "%s%s%s:%13s%13s%13s%13u\n", WHITESPACE(shiftlen + 2), "Transactions",
            WHITESPACE(25 - strlen("Transactions")), "", "", "", count);
    ...
    for (i = 0; i < m_stats_numofstats; i++) {
        ...
        fprintf(fout, "%s%s%s:%13u%13.2f%13u%13u\n", WHITESPACE(shiftlen + 2), stats_strings[i],
                WHITESPACE(25 - strlen(stats_strings[i])), min, mean, max, total);
    }
}

static void stats_threadstat_print(FILE *fout, m_stats_threadstat_t *threadstat) {
...
static void stats_summarize_all(m_statsmgr_t *statsmgr, m_stats_threadstat_t *summary) {
    m_stats_threadstat_t *threadstat;
    m_chhash_iter_t iter;
    m_stats_statset_t *statset;
    ...
    for (threadstat = statsmgr->alloc_threadstat_list_head; threadstat;
         threadstat = threadstat->next) {
        m_chhash_iter_init(threadstat->stats_table, &iter);
        while (M_R_SUCCESS == m_chhash_iter_next(&iter, &key, &value)) {
            statset = (m_stats_statset_t *)value;
            result = stats_get_statset(summary->stats_table, statset->name, &statset_summary);
            if (result != M_R_SUCCESS) {
                m_stats_statset_create(&statset_summary);
                m_stats_statset_init(statset_summary, statset->name);
                m_chhash_add(summary->stats_table, (m_chhash_key_t)statset->name,
            ...
            for (i = 0; i < m_stats_numofstats; i++) {
                ...
                statset_summary->stats[i].min =
                    MIN(statset_summary->stats[i].min, statset->stats[i].min);
                statset_summary->stats[i].max =
                    MAX(statset_summary->stats[i].max, statset->stats[i].max);

                summary->summary_statset.stats[i].total += statset->stats[i].total;
                summary->summary_statset.stats[i].min =
                    MIN(summary->summary_statset.stats[i].min, statset->stats[i].min);
                summary->summary_statset.stats[i].max =
                    MAX(summary->summary_statset.stats[i].max, statset->stats[i].max);
            }
        }
    }
}
...
void m_stats_print(m_statsmgr_t *statsmgr) {
    ...
    m_chhash_create(&(summary.stats_table), M_STATS_THREADSTAT_HASHTABLE_SIZE, false);
