...
int mtm_useraction_list_alloc(mtm_user_action_list_t **listp) {
    ...
    if (list) {
        list->array =
            (mtm_user_action_t *)realloc(NULL, sizeof(mtm_user_action_t) * USERACTION_LIST_SIZE);
...
void mtm_useraction_addUserCommitAction(mtm_tx_t *tx, _ITM_userCommitFunction fn,
    ...
    if (list->nb_entries == list->size) {
        list->size *= 2;
        list->array =
            (mtm_user_action_t *)realloc(list->array, sizeof(mtm_user_action_t) * list->size);
    }
...
void mtm_useraction_addUserUndoAction(mtm_tx_t *tx, const _ITM_userUndoFunction fn, void *arg) {
