...
acc_retransmit_latest_accept() {
	...
	volatile int lexical_state = 0;
    acceptor_record * rec;
    
    //Fetch the highest instance accepted
    sendbuf_clear(to_learners, accept_acks, this_acceptor_id);

acc_retransmit_latest_accept_xact_begin:
	MTM_XACT_BEGIN(acc_retransmit_latest_accept_xact)

	if (lexical_state == 2) {
		// Must complete the outstanding call before continuing
	    sendbuf_add_accept_ack_lexical(to_learners, rec, 2);
		lexical_state == 0;
	}	
    
	rec = stablestorage_get_record(highest_accepted_iid);
    //And retransmit it to learners
    if (sendbuf_add_accept_ack_lexical(to_learners, rec, 0) < 0) {
        /* must commit the transaction and flush the send buffer */
		goto commit_and_flush;
	}

	MTM_XACT_END(acc_retransmit_latest_accept_xact)
    ...
    /* unreachable */
commit_and_flush:
    sendbuf_add_accept_ack_lexical(to_learners, rec, 1);
	lexical_state = 2;
	goto acc_retransmit_latest_accept_xact_begin;
...
hande_prepare_req_batch_xact_begin:
    //Wrap changes in a  transaction
	MTM_XACT_BEGIN(handle_prepare_req_batch_xact)

 	if (lexical_state == 2) {
		// Must complete the outstanding call before continuing
	    sendbuf_add_prepare_ack_lexical(to_proposers, rec, 2);
		lexical_state == 0;
	}	

    //Iterate over the prepare_req in the batch
    for(i = next_i; i < prb->count; i++) {
        pr = &prb->prepares[i];
        ...
        if(rec != NULL) {
	        if (sendbuf_add_prepare_ack_lexical(to_proposers, rec, 0) < 0) {
				/* must commit the transaction and flush the send buffer */
				next_i = i+1; 
				goto commit_and_flush;
			}
	...
	MTM_XACT_END(handle_prepare_req_batch_xact)
    ...
    /* unreachable */
commit_and_flush:
    sendbuf_add_prepare_ack_lexical(to_proposers, rec, 1);
	lexical_state = 2;
	goto hande_prepare_req_batch_xact_begin;
...
handle_accept_req_batch_xact_begin:
    //Wrap in a transaction
   	MTM_XACT_BEGIN(handle_accept_req_batch_xact)

	if (lexical_state == 2) {
		// Must complete the outstanding iteration before continuing
	    sendbuf_add_accept_ack_lexical(to_learners, rec, 2);
    ...
    for(i = next_i; i < arb->count; i++) {
        ...
        if(rec != NULL) {
			if (sendbuf_add_accept_ack_lexical(to_learners, rec, 0) < 0) {
				/* must commit the transaction and flush the send buffer */
				next_i = i+1; /* save the current i to continue from */
				goto commit_and_flush;
			}
        }
   	...
   	MTM_XACT_END(handle_accept_req_batch_xact)
...
commit_and_flush:
    sendbuf_add_accept_ack_lexical(to_learners, rec, 1);
	lexical_state = 2;
	goto handle_accept_req_batch_xact_begin;
...
hande_repeat_req_batch_xact_begin:
    //Wrap in a (read-only) transaction
	MTM_XACT_BEGIN(handle_repeat_req_batch_xact)
	...
	MTM_XACT_END(handle_repeat_req_batch_xact)
...
commit_and_flush:
    sendbuf_add_accept_ack_lexical(to_learners, rec, 1);
	lexical_state = 2;
	goto hande_repeat_req_batch_xact_begin;
...
acc_deliver_callback(char * value, size_t size, iid_t iid, ballot_t ballot, int proposer) {
	...
	MTM_XACT_BEGIN(acc_deliver_callback_xact)
    stablestorage_save_final_value(value, size, iid, ballot);
	MTM_XACT_END(acc_deliver_callback_xact)
#endif
}
