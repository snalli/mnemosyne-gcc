...
//Adds a prepare_ack to the current message (a prepare_ack_batch)
int sendbuf_add_prepare_ack_lexical(udp_send_buffer * sb, acceptor_record * rec, int state) {
    paxos_msg * m = (paxos_msg *) &sb->buffer;
    assert(m->type == prepare_acks);    
...
//Adds an accept_ack to the current message (an accept_ack_batch)
int sendbuf_add_accept_ack_lexical(udp_send_buffer * sb, acceptor_record * rec, int state) {    
    paxos_msg * m = (paxos_msg *) &sb->buffer;
    assert(m->type == accept_acks);
	...
	if (state == 0) {
	    if(PAXOS_MSG_SIZE(m) + aa_size >= MAX_UDP_MSG_SIZE) {
    	    // Next accept to add does not fit, inform caller to flush
        	// message before remaking the call to add it
			return -1;
	    }
	}
