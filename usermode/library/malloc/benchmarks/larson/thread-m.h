...
/* One out of _LIBC, USE_PTHREADS, USE_THR, USE_SPROC or WIN32 should be
...
#elif WIN32 /* WIN32 threads + Paul's spin locks*/
...
_inline int EnterCS( LPCRITICAL_SECTION pcs )
{ EnterCriticalSection(pcs);
	return(0) ;
}
_inline int TryEnterCS( LPCRITICAL_SECTION pcs )
{ int val = TryEnterCriticalSection(pcs);
	return(!val) ;
}
_inline int LeaveCS( LPCRITICAL_SECTION pcs )
...
#define mutex_lock(m)              EnterCS(m)  
#define mutex_trylock(m)           TryEnterCS(m)
#define mutex_unlock(m)            LeaveCS(m)

#else

typedef splock_t mutex_t;

#define MUTEX_INITIALIZER          { L_UNLOCKED }
#define mutex_init(m)              S_INIT_LOCK(m)
#define mutex_lock(m)              S_LOCK(m)
#define mutex_trylock(m)           S_TRY_LOCK(m)
#define mutex_unlock(m)            S_UNLOCK(m)

#endif

#define tsd_key_t __declspec(thread) Void_t *
