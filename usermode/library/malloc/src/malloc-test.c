...
#define PROB_MEMALIGN 3
#endif
#ifndef PROB_REALLOC
#define PROB_REALLOC 70
#endif
#ifndef PROB_CALLOC
#define PROB_CALLOC 30
#endif
...
#endif
{
        ...
        if(size == 0) return 0;
        if(size > sizeof(unsigned long)) {
                if(*(unsigned long *)ptr != ((unsigned long)ptr ^ size)) return 1;
                i = TEST_INC;
        } else
                i = 0;
        for(; i<size; i+=TEST_INC) {
        ...
        j = (unsigned long)ptr ^ (size-1);
        if(ptr[size-1] != ((j ^ (j>>8)) & 0xFF)) return 3;
        return 0;
}
...
#endif
{
        ...
        if(mem_check(m->ptr, m->size)) {
                printf("memory corrupt at %p, size=%lu!\n", m->ptr, m->size);
                exit(1);
        }
#endif
        total_size -= m->size;
