...
wbetl_rollback(mtm_tx_t *tx)
{
	...
	PRINT_DEBUG("==> wbetl_rollback(%p[%lu-%lu])\n", tx, 
...
uint32_t
mtm_wbetl_beginTransaction_internal (mtm_tx_t *tx, 
                                     uint32_t prop, 
...
mtm_wbetl_rollbackTransaction (mtm_tx_t *tx, const _ITM_srcLocation *loc)
{
...
void _ITM_CALL_CONVENTION
mtm_wbetl_abortTransaction (mtm_tx_t *tx, 
                            ...
                            const _ITM_srcLocation *loc)
{
	...
	if (reason == userAbort) {
		...
		mtm_wbetl_restart_transaction(tx, RESTART_USER_RETRY);
	}
}
...
mtm_wbetl_tryCommitTransaction (mtm_tx_t *tx, _ITM_srcLocation *loc)
{
...
void _ITM_CALL_CONVENTION
mtm_wbetl_commitTransactionToId(mtm_tx_t *tx, 
