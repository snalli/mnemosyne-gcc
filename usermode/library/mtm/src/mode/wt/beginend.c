...
wt_rollback(mtm_tx_t *tx)
{
	...
	PRINT_DEBUG("==> wt_rollback(%p[%lu-%lu])\n", tx, 
...
uint32_t
mtm_wt_beginTransaction_internal (mtm_tx_t *tx, 
                                     uint32_t prop, 
...
mtm_wt_rollbackTransaction (mtm_tx_t *tx, const _ITM_srcLocation *loc)
{
...
void ITM_NORETURN
mtm_wt_restart_transaction (mtm_tx_t *tx, mtm_restart_reason r)
{
	uint32_t actions;
...
void _ITM_CALL_CONVENTION
mtm_wt_abortTransaction (mtm_tx_t *tx, 
                            ...
                            const _ITM_srcLocation *loc)
{
	...
	if (reason == userAbort) {
		...
		mtm_wt_restart_transaction(tx, RESTART_USER_RETRY);
	}
}
...
mtm_wt_tryCommitTransaction (mtm_tx_t *tx, _ITM_srcLocation *loc)
{
...
void _ITM_CALL_CONVENTION
mtm_wt_commitTransactionToId(mtm_tx_t *tx, 
