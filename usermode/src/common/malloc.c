...
__attribute__ ((section("PERSISTENT"))) struct malloc_bin  av[NBINS] = 
{
...
__attribute__ ((section("PERSISTENT"))) mbinptr maxClean = FIRSTBIN;
...
__attribute__ ((section("PERSISTENT"))) uint64_t persistent_heap = 0x0;
__attribute__ ((section("PERSISTENT"))) size_t sbrk_limit = 0; 

void *persistent_sbrk(size_t size) {
	if (persistent_heap == 0x0) {
		mnemosyne_segment_create(0xa00000000, 1024*1024, 0, 0);
		persistent_heap = 0xa00000000;
		if ((void *) persistent_heap == -1) {
			abort();
		}
	}
	sbrk_limit += size;
	return (void *) (persistent_heap + sbrk_limit);
}	
