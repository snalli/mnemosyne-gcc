...
static inline 
int 
wt_extend (mtm_tx_t *tx, mode_data_t *modedata)
{
	mtm_word_t now;

	PRINT_DEBUG("==> wt_extend(%p[%lu-%lu])\n", tx, 
...
static inline 
w_entry_t *
wt_write(mtm_tx_t *tx, 
            volatile mtm_word_t *addr, 
            mtm_word_t value,
            mtm_word_t mask)
{
	assert(tx->mode == MTM_MODE_wt);
...
# endif /* INTERNAL_STATS */
				mtm_wt_restart_transaction (tx, RESTART_REALLOCATE);
			}
			w = &modedata->w_set.entries[modedata->w_set.nb_entries];
...
#endif /* INTERNAL_STATS */
		mtm_wt_restart_transaction (tx, RESTART_LOCKED_WRITE);
	} else {
		/* Not locked */
...
# ifdef INTERNAL_STATS
			tx->aborts_rollover++;
# endif /* INTERNAL_STATS */
			mtm_wt_restart_transaction (tx, RESTART_VALIDATE_WRITE);
		}
...
//static inline
mtm_word_t 
mtm_wt_load(mtm_tx_t *tx, volatile mtm_word_t *addr)
{
	assert(tx->mode == MTM_MODE_wt);
	mode_data_t         *modedata = (mode_data_t *) tx->modedata[tx->mode];
	volatile mtm_word_t *lock;
...
# ifdef READ_LOCKED_DATA
		...
		if (version >= modedata->start && 
		    (version <= modedata->end || (tx->can_extend && wt_extend(tx, modedata)))) {
			/* Success */
#  ifdef INTERNAL_STATS
			tx->locked_reads_ok++;
#  endif /* INTERNAL_STATS */
...
give_up:
...
#endif /* INTERNAL_STATS */
		mtm_wt_restart_transaction (RESTART_LOCKED_READ);
	} else {
		/* Not locked */
		...
		if (version > modedata->end) {
			...
			if (!tx->can_extend || !wt_extend(tx, modedata)) {
				/* Not much we can do: abort */
#if CM == CM_PRIORITY
				/* Abort caused by invisible reads */
				tx->visible_reads++;
...
#endif /* INTERNAL_STATS */
				mtm_wt_restart_transaction (RESTART_VALIDATE_READ);
			}
 ...
 */
void mtm_wt_store(mtm_tx_t *tx, volatile mtm_word_t *addr, mtm_word_t value)
{
	wt_write(tx, addr, value, ~(mtm_word_t)0);
}
 ...
 */
void mtm_wt_store2(mtm_tx_t *tx, volatile mtm_word_t *addr, mtm_word_t value, mtm_word_t mask)
{
	wt_write(tx, addr, value, mask);
}



DEFINE_LOAD_BYTES(wt)
DEFINE_STORE_BYTES(wt)

FOR_ALL_TYPES(DEFINE_READ_BARRIERS, wt)
FOR_ALL_TYPES(DEFINE_WRITE_BARRIERS, wt)
