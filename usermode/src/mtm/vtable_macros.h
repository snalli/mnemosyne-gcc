...
#  define FOREACH_OUTER_FUNCTION(ACTION, ...)                                            \
    ACTION (void,     abortTransaction,     (mtm_thread_t *td,                       \
                                             _ITM_abortReason reason,                    \
                                             const _ITM_srcLocation *loc), __VA_ARGS__)  \
    ACTION (void,     rollbackTransaction,  (mtm_thread_t *td,                       \
                                             const _ITM_srcLocation *loc), __VA_ARGS__)  \
    ACTION (void,     commitTransaction,    (mtm_thread_t *td,                       \
                                             const _ITM_srcLocation *loc), __VA_ARGS__)  \
    ACTION (uint32_t, tryCommitTransaction, (mtm_thread_t *td,                       \
                                             const _ITM_srcLocation *loc), __VA_ARGS__)  \
    ACTION (void,     commitTransactionToId,(mtm_thread_t *td,                       \
                                             const _ITM_transactionId tid,               \
                                             const _ITM_srcLocation *loc), __VA_ARGS__)  \
    ACTION (uint32_t, beginTransaction,     (mtm_thread_t *td,                       \
                                             uint32 __properties,                        \
                                             const _ITM_srcLocation *loc), __VA_ARGS__)  \
